...
BaseSQL.prototype.exists = function (model, id, callback) {
    var sql = 'SELECT 1 FROM ' + this.tableEscaped(model) + ' WHERE ' + this.escapeName('id') + ' = ' + this.escapeId(id) + ' LIMIT 1';
...
BaseSQL.prototype.find = function find(model, id, callback) {
    ...
    var sql = 'SELECT * FROM ' + this.tableEscaped(model) + ' WHERE ' + this.escapeName('id') + ' = ' + idNumber + ' LIMIT 1';
    ...
    this.query(sql, function (err, data) {
        ...
        callback(err, _this.fromDatabase(model, data[0]));
    });
};

BaseSQL.prototype.destroy = function destroy(model, id, callback) {
    var sql = 'DELETE FROM ' + this.tableEscaped(model) + ' WHERE ' + this.escapeName('id') + ' = ' + this.escapeId(id);
...
BaseSQL.prototype.count = function count(model, callback, where) {
    ...
    this.queryOne('SELECT count(*) as cnt FROM ' + this.tableEscaped(model) + ' ' + buildWhere(where), function (err, res) {
        if (err) {
            return callback(err);
        }

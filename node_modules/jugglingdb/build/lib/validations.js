...
function validateUniqueness(attr, conf, err, done) {
    ...
    this.constructor.all(cond, function (error, found) {
        ...
        if (found.length > 1) {
            err();
        } else if (found.length === 1 && (!_this.id || !found[0].id || found[0].id.toString() !== _this.id.toString())) {
            err();
        }
        done();
    });
...
Validatable.prototype.isValid = function (callback, data) {
    ...
    if (callback.length < 2) {
        ...
        callback = function callback(err, valid) {
            _callback(valid);
        };
    }
    ...
    this.trigger('validate', function (validationsDone) {
        ...
        this.constructor._validations.forEach(function (v) {
            if (v[2] && v[2].async) {
                ...
                wait += 1;
                process.nextTick(function () {
                    return validationFailed(inst, v, done);
                });
            } else if (validationFailed(inst, v)) {
                valid = false;
            }
        });
        ...
        if (!async) {
            validationsDone.call(inst, function () {
                ...
                if (callback) {
                    callback(valid ? null : new ValidationError(inst), valid);
                }
            });
        }
        ...
        function done(fail) {
            ...
            if (wait === 0) {
                validationsDone.call(inst, function () {
                    ...
                    if (callback) {
                        callback(valid ? null : new ValidationError(inst), valid && !asyncFail);
                    }
                });
            }
        }
...
function configure(cls, validation, args, opts) {
    ...
    args.forEach(function (attr) {
        return cls._validations.push([attr, conf, opts]);
    });
}
...
Errors.prototype.__localize = function localize(locale) {
    var codes = this.__codes;
    var errors = this;
    var result = {};
    var i18n = this.__obj.constructor.i18n;
    var v = i18n && i18n[locale] && i18n[locale].validation;
